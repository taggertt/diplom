...
+ Первичная оплата картой со статусом APPROVED
  ...
  * В после год ввести двузначный год сгенерированный случайным образом в значениях 25-29
...
### **Для оплаты тура по карте**
  ...
  + Пустое поле номер карты
    ...
    **Ожидаемый результат:** Под полем номер карты появляется подсказка "неверный формат". Запрос не отправляется.

  + Пустое поле месяц
    * В поле номер карты ввести 4444 4444 4444 4441 (соответствует APPROVED в файле data.json)
    ...
    **Ожидаемый результат:** Под полем месяц появляется подсказка "неверный формат". Запрос не отправляется.
  ...
  + Пустое поле год
    ...
    **Ожидаемый результат:** Под полем год появляется подсказка "неверный формат". Запрос не отправляется.
  ...
  + Пустое поле cvc/ccv 
    ...
    * В после год ввести двузначный год сгенерированный случайным образом в значениях 25-29
    * В поле Владелец ввести 5 латинских символов сгенерированных с помощью faker
    * Поле cvc/ccv оставить пустым
    * Нажать кнопку "продолжить"
   
    **Ожидаемый результат:** Под полем cvc/ccv появляется подсказка "неверный формат". Запрос не отправляется.
    
  + Ввод невалидного месяца по дате
    * В поле номер карты ввести 4444 4444 4444 4441 (соответствует APPROVED в файле data.json)
  ...
  + Ввод невалидного месяца (не учитывает логику)
    * В поле номер карты ввести 4444 4444 4444 4441 (соответствует APPROVED в файле data.json)
    * В поле месяц ввести номер "13"
    * В после год ввести двузначный год сгенерированный случайным образом в значениях 25-29
    * В поле Владелец ввести 5 латинских символов сгенерированных с помощью faker
    * В поле cvc/ccv ввести трёхзначный номер сгенерированный случайным образом
    * Нажать кнопку "продолжить"
   
    **Ожидаемый результат:** Под полем месяц появлется подсказка "неверный формат". Запрос не отправляется.
...
+ Ввод владельца кириллицей
  * В поле номер карты ввести 4444 4444 4444 4441 (соответствует APPROVED в файле data.json)
  * В поле месяц ввести номер месяца сгенерированный случайным образом (01-12)
  * В после год ввести двузначный год сгенерированный случайным образом в значениях 25-29
...
+ Ввод двух цифр в поле cvc/ccv
  * В поле номер карты ввести 4444 4444 4444 4441 (соответствует APPROVED в файле data.json)
  * В поле месяц ввести номер месяца сгенерированный случаным образом (01-12)
  * В после год ввести двузначный год сгенерированный случайным образом в значениях 25-29
  * В поле Владелец ввести 5 латинских символов сгенерированных с помощью faker
  * В поле cvc/ccv ввести двузначный номер сгенерированный случайным образом
  * Нажать кнопку "продолжить"

  **Ожидаемый результат:** Под полем cvc/ccv появляется подсказка "неверный формат"
...
# 4.	Расчёт времени.
Время на сборку базы и написание тестов и отчётов, последующим выкладыванием на GitHub и поднятием Appveyor – 120 часов.
